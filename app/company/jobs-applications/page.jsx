...
const JobsApplications = () => {
  ...
  const fetchJobApplication = async () => {
    try {
      ...
      const transformedData = response.data.map((item) => {
        ...
        //   postedBy: item.posted_by, // This is the key field we need
        // });
        ...
        return {
          ...
          hasResume: !!applicant.resume,
          jobId: item.id,
          postedBy: item.posted_by, // ✅ Store posted_by (ID: 45 in your case)
          applicationDetails: item,
        };
      });
  ...
  // NEW: Handle Message Button Click - FIXED VERSION
  const handleMessage = async (application) => {
    try {
      setMessagingApplicant(application.id);

      // console.log(
      //   "📤 Attempting to create chat with application:",
      //   application
      // );
      // console.log("📤 Posted by (job poster ID):", application.id);

      // Validate that postedBy exists
      if (!application.id) {
        throw new Error("Posted by ID is missing from application data");
      }

      // Step 1: Create or get existing chat with the job poster (posted_by)
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/chat/create`,
        {
          user_id: application.id, // ✅ Use postedBy field (camelCase)
        },
        { withCredentials: true }
      );
      ...
      // Step 2: Extract chat ID from response
      const chatId =
        response.data.id || response.data.chat_id || response.data.chatId;

      if (!chatId) {
        throw new Error("Chat ID not found in response");
      }

      // Step 3: Store chat ID in sessionStorage so ChatApp can auto-open it
      sessionStorage.setItem("openChatId", chatId.toString());
      sessionStorage.setItem("openChatUser", application.name);

      toast.success(`Opening chat with job poster...`);

      // Step 4: Redirect to chat page
      // The ChatApp component will automatically open this chat
      router.push("/company/applicant-messages");
    } catch (error) {
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      ...
                      <Button
                        ...
                        disabled={messagingApplicant === application.id}
                        className="bg-transparent border-gray-600 text-white hover:bg-blue-600 hover:text-white hover:border-blue-600 disabled:opacity-50"
                      >
                        {messagingApplicant === application.id ? (
                          <>
                            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                            Opening...
                          </>
