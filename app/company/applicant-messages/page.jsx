...
export default function ChatApp() {
  ...
  const messagesEndRef = useRef(null);
  const typingTimeoutRef = useRef(null);
  const hasAutoOpened = useRef(false); // Track if we've already auto-opened a chat
  ...
  // NEW: Auto-open chat from sessionStorage (called after chats are loaded)
  useEffect(() => {
    if (chats.length > 0 && !hasAutoOpened.current) {
      const chatIdToOpen = sessionStorage.getItem('openChatId');
      const chatUserName = sessionStorage.getItem('openChatUser');

      if (chatIdToOpen) {
        console.log("🔍 Auto-opening chat ID:", chatIdToOpen);
        
        // Find the chat in the list
        const chatToOpen = chats.find(
          (chat) => String(chat.id) === String(chatIdToOpen)
        );

        if (chatToOpen) {
          console.log("✅ Found chat to auto-open:", chatToOpen);
          selectChat(chatToOpen);
          hasAutoOpened.current = true; // Mark as opened
          
          // Clean up sessionStorage
          sessionStorage.removeItem('openChatId');
          sessionStorage.removeItem('openChatUser');
        } else {
          console.warn("⚠️ Chat not found in list, may need to refresh");
          // Optionally refresh chats if not found
          refreshChats().then(() => {
            const retryChat = chats.find(
              (chat) => String(chat.id) === String(chatIdToOpen)
            );
            if (retryChat) {
              selectChat(retryChat);
