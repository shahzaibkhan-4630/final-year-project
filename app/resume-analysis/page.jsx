...
const ResumeAnalysis = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // --- PARSE CLAUDE CHANGES STRING ---
      const parseAnalysisString = (text) => {
        if (!text) return { additions: [], changes: [], removals: [] };

        const sections = text
          .split(/\d\.\s*(ADDITIONS|CHANGES|REMOVALS)\s*-\s*/i)
          .slice(1);
        const result = { additions, changes, removals };

        for (let i = 0; i < sections.length; i += 2) {
          const key = sections[i].toLowerCase();
          const content = sections[i + 1]
            .split(/\s*-\s+/) // split on bullet dashes
            .map((s) => s.trim())
            .filter(Boolean);
          result[key] = content;
        }
      ...
      const parsedChanges = parseAnalysisString(response.data.changes || "");

      setSuccess(true);
      setAnalysisResult({
        ...response.data.analysis,
        ...parsedChanges, // now additions, changes, removals are arrays
      });
    } catch (err) {
      console.error("Error analyzing resume:", err);
          ...
          {analysisResult && (
            <div className="mt-8 space-y-6">
              ...
              >
                ...
                <h2 className="text-[#DDDFFF] text-3xl sm:text-4xl font-bold font-dm-sans mb-2">
                  ATS Score
                </h2>
                <div
                  className={`text-6xl sm:text-7xl font-bold font-dm-sans mb-4 ${getScoreColor(
                    analysisResult.ats_score
                  )}`}
                >
                  {analysisResult.ats_score}
                ...
                <p className="text-[#848484] text-base sm:text-lg max-w-2xl mx-auto font-dm-sans">
                  {analysisResult.overall_summary}
                </p>

                {/* Quick Stats */}
                {analysisResult.industry_match &&
                  analysisResult.experience_level && (
                    <div className="flex flex-wrap justify-center gap-4 mt-6">
                      <div className="bg-[#2A2D35] rounded-lg px-4 py-2">
                        <p className="text-[#848484] text-xs font-dm-sans mb-1">
                          Industry Match
                        </p>
                        <p className="text-[#DDDFFF] text-sm font-semibold font-dm-sans">
                          {analysisResult.industry_match}
                        </p>
                      </div>
                      <div className="bg-[#2A2D35] rounded-lg px-4 py-2">
              ...
              {analysisResult.section_scores && (
                ...
                >
                  <div className="flex items-start gap-3 mb-6">
                    ...
                    <div>
                      <h3 className="text-purple-400 text-lg font-semibold font-dm-sans mb-1">
                        Section Breakdown
                      </h3>
                      <p className="text-[#848484] text-sm font-dm-sans">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.entries(analysisResult.section_scores).map(
                      ([section, score]) => (
                        <div
                          key={section}
                          className="bg-[#2A2D35] rounded-lg p-4"
                        >
                          <div className="flex justify-between items-center mb-2">
                            <p className="text-[#DDDFFF] text-sm font-medium font-dm-sans capitalize">
                              {section.replace(/_/g, " ")}
                            </p>
                            <span
                              className={`text-lg font-bold font-dm-sans ${getScoreColor(
                                score
                              )}`}
                            >
                              {score}
              ...
              {/* Top Recommendations */}
              {analysisResult.top_recommendations &&
                analysisResult.top_recommendations.length > 0 && (
                  <div
                    className="rounded-2xl p-6 sm:p-8"
                    style={{
                      background:
                  ...
                  >
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div>
                        <h3 className="text-yellow-400 text-lg font-semibold font-dm-sans mb-1">
                          Top Priority Actions
                        </h3>
                        <p className="text-[#848484] text-sm font-dm-sans">
                          Start with these critical improvements
                        </p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      {analysisResult.top_recommendations.map((rec, index) => (
                        <div
                          key={index}
                          className="bg-[#2A2D35] rounded-lg p-4 flex items-start gap-3"
                        >
                          <div className="bg-yellow-500/20 rounded-full w-8 h-8 flex items-center justify-center shrink-0 mt-0.5">
                            <span className="text-yellow-400 font-bold font-dm-sans text-sm">
                              {index + 1}
                            </span>
                          </div>
                          <p className="text-[#E1E4F0] text-sm sm:text-base font-dm-sans leading-relaxed">
                            {rec}
                          </p>
                        </div>
                      ))}
              ...
              {analysisResult.strengths &&
                analysisResult.strengths.length > 0 && (
                  ...
                  >
                    ...
                    <div className="bg-[#2A2D35] rounded-lg p-5 sm:p-6 space-y-3">
                ...
                analysisResult.weaknesses.length > 0 && (
                  ...
                  >
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div>
                        <h3 className="text-orange-400 text-lg font-semibold font-dm-sans mb-1">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {analysisResult.keyword_analysis.missing_keywords &&
                      analysisResult.keyword_analysis.missing_keywords.length >
                        0 && (
                        <div className="bg-[#2A2D35] rounded-lg p-4">
                          <p className="text-red-400 text-sm font-semibold font-dm-sans mb-3">
                            Missing Keywords
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {analysisResult.keyword_analysis.missing_keywords.map(
                              (keyword, index) => (
                                <span
                                  key={index}
                                  className="bg-red-500/10 border border-red-500/30 text-red-400 px-3 py-1 rounded-full text-xs font-dm-sans"
                                >
                                  {keyword}
                                </span>
                              )
                    ...
                    {/* Present Keywords */}
                    {analysisResult.keyword_analysis.present_keywords &&
                      analysisResult.keyword_analysis.present_keywords.length >
                        0 && (
                        <div className="bg-[#2A2D35] rounded-lg p-4">
                          <p className="text-green-400 text-sm font-semibold font-dm-sans mb-3">
                            Present Keywords
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {analysisResult.keyword_analysis.present_keywords.map(
                              (keyword, index) => (
                                <span
                                  key={index}
                                  className="bg-green-500/10 border border-green-500/30 text-green-400 px-3 py-1 rounded-full text-xs font-dm-sans"
                                >
                                  {keyword}
                                </span>
                              )
                    ...
                    {/* Industry Terms */}
                    {analysisResult.keyword_analysis.industry_specific_terms &&
                      analysisResult.keyword_analysis.industry_specific_terms
                        .length > 0 && (
                        <div className="bg-[#2A2D35] rounded-lg p-4">
                          <p className="text-blue-400 text-sm font-semibold font-dm-sans mb-3">
                            Industry Terms
                          </p>
                          <div className="space-y-2">
                            {analysisResult.keyword_analysis.industry_specific_terms.map(
                              (term, index) => (
                                <p
                                  key={index}
                                  className="text-[#E1E4F0] text-xs font-dm-sans"
                                >
                                  {term}
                                </p>
                              )
              ...
              {/* Additions */}
              {analysisResult.additions &&
                analysisResult.additions.length > 0 && (
                  <div
                    className="rounded-2xl p-6 sm:p-8"
                    style={{
                      background:
                  ...
                  >
                    ...
                    <div className="bg-[#2A2D35] rounded-lg p-5 sm:p-6 space-y-3">
                      {analysisResult.additions.map((addition, index) => (
                        <div key={index} className="flex items-start gap-3">
                          <div className="bg-green-500/20 rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5">
                ...
                >
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[#848484] text-sm font-dm-sans">
                        Update these areas for better impact
                      </p>
                    </div>
                  </div>
                  <div className="bg-[#2A2D35] rounded-lg p-5 sm:p-6 space-y-3">
                    {analysisResult.changes.map((change, index) => (
                      <div key={index} className="flex items-start gap-3">
                        <div className="bg-blue-500/20 rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5">
              ...
              {/* Removals */}
              {analysisResult.removals &&
                analysisResult.removals.length > 0 && (
                  <div
                    className="rounded-2xl p-6 sm:p-8"
                    style={{
                      background:
                  ...
                  >
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-[#848484] text-sm font-dm-sans">
                          Eliminate these for a cleaner resume
                        </p>
                      </div>
                    </div>
                    <div className="bg-[#2A2D35] rounded-lg p-5 sm:p-6 space-y-3">
                      {analysisResult.removals.map((removal, index) => (
                        <div key={index} className="flex items-start gap-3">
                          <div className="bg-red-500/20 rounded-full w-6 h-6 flex items-center justify-center shrink-0 mt-0.5">
